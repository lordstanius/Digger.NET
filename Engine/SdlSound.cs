...
namespace Digger.Net
{
    public class SdlSound : ISoundDevice
    {
        ...
        public bool SetDevice(ushort sampleRate, ushort bufferSize, GetSampleDelegate getSample)
        {
            ...
            sud.lp_fltr = Filter.bqd_lp_init(sud.obtained.freq, 4000).ToPointer();
            sud.hp_fltr = Filter.bqd_hp_init(sud.obtained.freq, 1000).ToPointer();
        ...
        private static void FillAudio(IntPtr udata, IntPtr stream, int len)
        {
            ...
            for (int i = 0; i < len / sizeof(short); i++)
            {
                ...
                sample = Filter.bqd_apply(ref hp_fltr, (sample - 127.0) * 128.0);
                sample = Filter.bqd_apply(ref lp_fltr, sample);
