...
namespace Digger.Net
{
    public class SdlTimer
    {
        ...
        public SdlTimer()
        {
            ...
            loop_error = DigMath.recfilter_init(tfreq, 0.1);
            DigMath.PFD_init(ref phase_detector, 0.0);
        ...
        public void SyncFrame()
        {
            ...
            double eval = DigMath.PFD_get_error(ref phase_detector, clk_rl);
            double filterval;
            if (eval != 0)
                filterval = DigMath.recfilter_apply(ref loop_error, DigMath.sigmoid(eval));
            else
                filterval = DigMath.recfilter_getlast(ref loop_error);

            double add_delay_d = (DigMath.freqoff_to_period(tfreq, 1.0, filterval) * 1000.0) + cum_error;
